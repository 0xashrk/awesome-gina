...
- For recipe submissions, include a `Quick Copy Prompt (Ask Gina)` section with a fenced `text` code block that begins with `promptText:` and includes `- Execute with: Ask Gina agent`.
...
- quick-copy `promptText` block with execution agent line (for recipe)
...
- Recipe submissions include a noob-friendly `Quick Copy Prompt (Ask Gina)` `promptText` block with an explicit execution agent line.
